    ...
    <div class="hero-caption">
      <div class="container fg-white h-100">
        <div class="row justify-content-center align-items-center text-center h-100">
          <div class="col-lg-6">
            <h3 class="mb-4 fw-medium"><?=ucfirst(session('kategori'))?></h3>
    ...
    <div class="container">
      <div class="row">
        <div class="col-lg-8 py-3">
          <h3><?=ucfirst(session('kategori'))?></h3>
          <?php 
            if(empty($post)){
              echo 'Hmm.. tidak ada artikel dengan kategori '.ucfirst(session('kategori')).' :(';
              echo '<br>Mungkin kamu bisa baca postingan terbaru kami !';
              foreach ($recents as $recent) {
          ?>
            <article class="blog-entry widget-wrap">
              <div class="entry-header">
                <div class="post-thumbnail">
                  <img src="<?=base_url('assets/uploads/post/'.$recent->file_gambar)?>" style="width:100%">
                </div>
                <div class="post-date">
                  <h3><?=date('d', strtotime($recent->tgl_insert))?></h3>
                  <span><?=date('M', strtotime($recent->tgl_insert))?></span>
                  <span></span>
                </div>
              </div>
              <div class="post-title"><b><a href="<?=base_url('article/'.$recent->slug)?>" style="font-weight: bold"><?=ucfirst($recent->judul)?></a></b></div>
              <div class="entry-meta mb-2">
                <div class="meta-item entry-author">
                  <div class="icon" style="width:30px; height: 30px">
                    <img src="<?=base_url('assets/uploads/penulis/'.$recent->foto)?>" alt="" class="rounded-circle" style="max-width:30px">
                  </div>
                  by <a href="<?=base_url('writer/'.$recent->idpenulis)?>"><?=ucfirst($recent->nama_penulis)?></a>
                </div>
                <div class="meta-item">
                  <div class="icon">
                  ...
                  Category: 
                  <a href="<?=base_url('category/'.strtolower($recent->nama_kategori))?>"><?=ucfirst($p->nama_kategori)?></a>
                </div>
                <div class="meta-item">
                  <div class="icon">
                    <span class="mai-chatbubble-ellipses"></span>
                  </div>
                  <a href="#"><?=$recent->frekuensi?></a>
                </div>
              </div>
              <div class="entry-content">
                <?php
                  $konten = explode('</p>', $recent->isi_post);
                ?>
                <p><?=$konten[0]?></p>
              </div>
              <a href="<?=base_url('article/'.$recent->slug)?>" class="btn btn-primary">Continue Reading</a>
            </article>
          <?php
              } // end of foreach
            } // end of if
